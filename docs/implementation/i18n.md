...
| File | Purpose |
| --- | --- |
| `src/context/LanguageContext.js` | The context object itself (`createContext()`), nothing else |
| `src/context/LanguageProvider.jsx` | Provider component: state, toggle, translation function. Split from the context object so this file can export a component without breaking Vite Fast Refresh |
...
2. Import it in `LanguageProvider.jsx` and add it to the `translations` map
