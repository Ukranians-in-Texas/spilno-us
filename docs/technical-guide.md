...
**Companion docs:** [concepts.md](concepts.md) explains unfamiliar patterns anchored to how this project uses them — read proxy, RLS, serverless functions, JWT, and the data model. [walkthrough.md](walkthrough.md) traces each major feature end-to-end with annotated code excerpts. [testing.md](testing.md) covers how to run, add, and extend the test suite. Read this guide for the *what* and *why*; read those for the *how* and *how-it-connects*.
...
> See [concepts.md — Supabase Postgres](concepts.md#supabase-postgres) for how PostgREST works, [The `services` table](concepts.md#the-services-table) for a beginner-friendly walkthrough of the schema, and [Why a single table works here](concepts.md#why-a-single-table-works-here) for the trade-off rationale.
...
> See [concepts.md — The read proxy pattern](concepts.md#the-read-proxy-pattern) for why public reads go through `/api` instead of hitting Supabase directly, and [Serverless functions](concepts.md#serverless-functions) for how Vercel deploys the `api/` directory.
...
> See [concepts.md — Client vs. server validation](concepts.md#client-vs-server-validation--the-server-is-authoritative) for why validation is duplicated, and [Honeypot fields](concepts.md#honeypot-fields) for how the hidden field works.
...
> See [concepts.md — JWT](concepts.md#jwt-json-web-token) for how JWTs carry identity claims, [Row Level Security](concepts.md#row-level-security-rls) for how RLS policies enforce access in Postgres, and [Service key vs. anon key](concepts.md#service-key-vs-anon-key) for the two-key architecture.
...
> See [concepts.md — The Telegram bot](concepts.md#the-telegram-bot--two-roles-in-one) for the dual notification/action-handler role, and [HTML escaping](concepts.md#html-escaping-in-telegram-messages) for why `escapeHtml` exists and the replacement order.
...
> See [concepts.md — Unsigned Cloudinary uploads](concepts.md#unsigned-cloudinary-uploads) for the upload/delete asymmetry and why orphaned images exist.
...
- Admin dashboard pages (`AdminQueuePage`, `AdminServicesPage`, `EditPanel`) — no component or E2E coverage.
...
- Image upload flow, responsive/mobile menu — no E2E coverage.

See [testing.md — Coverage gaps](testing.md#coverage-gaps) for a prioritized list.
...
> See [concepts.md — HTTP headers](concepts.md#http-headers) for what each security header does and how functional headers (Cache-Control, Authorization, secret-token) work. [Service key vs. anon key — How isolation is enforced](concepts.md#how-isolation-is-enforced) covers the Vite `VITE_` prefix and Vercel runtime boundaries. [vercel.json](concepts.md#verceljson--what-it-does-and-why) explains the SPA rewrite rule and header configuration.
